...
export default function ProductsClient({ products }: ProductsClientProps) {
  ...
  // Filter products
  const filteredProducts = filter === 'all' ? products : products.filter((item) => item.category === filter);
  ...
  const handleLoadMore = () => {
    setDisplayCount((prev) => Math.min(prev + LOAD_MORE_COUNT, filteredProducts.length));
  };
      ...
      <div className="container-wide">
        ...
        {filteredProducts.length === 0 ? (
          ...
          <>
            ...
            {/* 全件表示時のメッセージ */}
            {!hasMore && filteredProducts.length > INITIAL_DISPLAY_COUNT && (
